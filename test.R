...
server <- function(input, output, session) {
  ...
  # selected country
  output$selected_country <- renderText({input$country})
  
  # exploration description
  output$exploration_description <- renderText({
    paste("The graph of", input$outcome, "and the three controls: age, education, and sex")})
  
  # regression table description
  output$regression_tab_description <- renderText({
    paste0("The regression model of ", input$outcome)
  })
  
  # regression scatter plot description
  output$regression_plot_description <- renderText({
    paste0("The scatter plot showing the predicted versus the residuals from the regression model of ", input$outcome)
  })
